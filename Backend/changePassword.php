...
$name=$b['name'];
$pass=$b['password'];
$existing=sha1($_POST['existing']);
$p1=sha1($_POST['p1']);
$p2=sha1($_POST['p2']);
if($_POST['existing']==NULL || $_POST['p1']==NULL || $_POST['p2']==NULL)
{
	//Do Nothing
}
else
...
if($existing!=$pass)
{
	$info="Incorrect existing Password";
}
elseif($p1!=$p2)
	{
...
<tr><td class="labels">existing Password :</td><td><input type="password" name="existing" size="25" class="fields" placeholder="Enter existing Password" required="required" /></td></tr>
